...
void Test_Audio_Task(void *arg) {
  ...
  while (1) {
    ...
    if (even & 0x40) {
      if (display_ready && c6_amoled::lvglLock(500)) {
...
void setup() {
  ...
  if (!display_ready) Serial.println("Display unavailable; audio test continues without UI.");

  bsp_spiffs_mount();

  CodecPort_ = new CodecPort(board.i2c(), "C6_AMOLED_2_16");
  Audio_start();
