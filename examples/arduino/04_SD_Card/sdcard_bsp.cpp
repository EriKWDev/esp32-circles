...
CustomSDPort::CustomSDPort(const char *SdName, int clk, int mosi, int miso, int cs, spi_host_device_t spihost)
  ...
  // The status UI initializes the LCD on SPI2 first. Reusing that host is expected.
  ...
  sdmmc_host_t host = SDSPI_HOST_DEFAULT();
  host.slot = spihost;
  esp_err_t mount_err = esp_vfs_fat_sdspi_mount(SdName_, &host, &slot_config, &mount_config, &sdCardHead);
  ...
  if (sdCardHead != NULL) {
    ...
    ESP_LOGE(TAG, "SD card mount failed: %s", esp_err_to_name(mount_err));
    is_SdcardInitOK = 0;
  }
}
