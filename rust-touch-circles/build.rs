...
fn emit_secrets(manifest: &Path, out_dir: &Path) {
    ...
    // A second network to fall back on when the first is nowhere to be found -
    // a phone hotspot, typically, which is what makes the panel useful away from
    // its own wall.
    ...
    let ssid4 = pick("WIFI_SSID4", "ssid4", "");
    let pass4 = pick("WIFI_PASS4", "pass4", "");
    ...
    writeln!(out, "pub const WIFI_SSID4: &str = {ssid4:?};").unwrap();
    writeln!(out, "pub const WIFI_PASS4: &str = {pass4:?};").unwrap();
