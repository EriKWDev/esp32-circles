...
- 两个保留固件路径均不会被默认仓库工作流修改、构建或打包。策略工作流会在不修改文件的
  情况下校验工厂镜像的文件集合、大小和 SHA-256 摘要。
...
这些哈希由可见的仓库策略工作流校验。它们不是数字签名，也不能证明真实性或硬件适用性。
不得在未记录来源、版本、预期烧录流程、新校验和及硬件验证结果时重命名、重新生成或替换镜像。
