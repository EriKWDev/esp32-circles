# Firmware artifacts

[简体中文](FIRMWARE_ZH.md)

Source snapshots, immutable recovery inputs, and generated example artifacts are
intentionally separate:

- `firmware/xiaozhi/` is a preserved upstream source snapshot labeled 2.2.5.
- `firmware/factory_firmware/` contains prebuilt recovery images retained from
  the original repository.
- `release-artifacts/` is generated by example CI and is never committed.
- neither preserved firmware path is modified, built, or packaged by the
  default repository workflows. The policy workflow verifies the factory-image
  file set, sizes, and SHA-256 digests without modifying it.

## Immutable recovery inventory

| File | Size | SHA-256 |
| --- | ---: | --- |
| `01_Fac-v1.0.0.bin` | 15,859,712 bytes | `1DD2BE304BEC964831EEC8484C67C97D7B56934343DEFC4EBCAB63ED84D32C33` |
| `02_xiaozhi-v2.2.5.bin` | 11,075,284 bytes | `7FDDD220C86AD793F360EF88FEC8454C2765CF9C596C76FBD0753D01889AF169` |

These hashes are checked by the visible repository-policy workflow. They are
not signatures and do not establish authenticity or hardware suitability. Do
not rename, regenerate, or replace an image without recording its source,
version, expected flash process, new checksum, and hardware validation result.

Use recovery images only with the offsets and procedure in the
[official product documentation](https://docs.waveshare.com/ESP32-C6-Touch-AMOLED-2.16).
A successful build of current XiaoZhi source would not certify that a historical
binary is reproducible from it. CI artifacts cover the example projects, not the
preserved firmware paths.

## Guided CI artifact flashing

`Flash-CI-Firmware.cmd` starts a Windows GUI for testing the 27 example bundles
from a clean, non-detached branch with exactly one open, non-draft pull request.
It only accepts successful workflow artifacts whose SHA matches local `HEAD`.
Install Git, authenticated GitHub CLI (`gh auth login`), Python with `esptool`,
and a USB serial driver first. Run `Flash-CI-Firmware.cmd -ListOnly` to inspect
the planned order, or `Flash-CI-Firmware.cmd -SelfTest` to check the local
contract without a device, authentication, or GUI. Use `-Port COMx` when the
tool cannot identify exactly one ESP32-C6 USB VID/PID device.

The GUI verifies each archive manifest, bundled file hash and size, and flash
range before it invokes `python -m esptool ... write_flash`. It never erases the
flash. After a verified write, test the board and select **Mark PASS and flash
next**; progress is stored per final SHA outside the repository. Example CI
artifacts are diagnostic test builds, not replacements for factory recovery
images or a factory flashing procedure.
